...
class dataFormat:
    def __init__(self):
        ...
        self.file_path = f"ipca/source/data/ipca_{self.ano_mes_formatado}.csv"

    def dataDownload(self):
        """
        Realiza o download dos dados em csv

        Returns
        ---
        wget download
        """
        if os.path.exists(f"ipca/source/data/ipca_{self.ano_mes_formatado}.csv"):
            return True
        else:
            return wget.download(
                url=self.url, out=f"ipca/source/data/ipca_{self.ano_mes_formatado}.csv"
            )

    def dataTransform(self) -> DataFrame:
        """
        Realiza as transformações dos dados
